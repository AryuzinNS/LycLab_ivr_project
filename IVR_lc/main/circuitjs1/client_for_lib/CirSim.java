...
//	Добавление элементов меню
	...
	Frame iFrame;
// Добавление рабочего окна 
    Canvas cv;
    Context2d cvcontext;
    Canvas backcv;
    Context2d backcontext;
    static final int MENUBARHEIGHT=30;
    static int VERTICALPANELWIDTH=200; // default
...
// Изменена конфигурация меню
	//меню сохранения
	fileMenuBar = new MenuBar(true);
	if (isElectron())
	    fileMenuBar.addItem(iconMenuItem("clone", "New Window...", new MyCommand("file", "newwindow")));
	fileMenuBar.addItem(LS("Clear editor window"), new MyCommand("file", "clearpage"));
...
//	aboutItem.setScheduledCommand(new MyCommand("file","about"));
	...
	buttonPanel=(VERTICALPANELWIDTH == 166) ? new VerticalPanel() : new VerticalPanel();
...
//	m.addItem(optionsItem = new CheckboxAlignedMenuItem(LS("Other Options..."), new MyCommand("options","other")));
	...
	speedBar.setStylePrimaryName("scrollbars");
	verticalPanel.add(speedBar);

	verticalPanel.add( l = new Label(LS("Current Speed")));
	l.addStyleName("topTopSpace");
	currentBar = new Scrollbar(Scrollbar.HORIZONTAL, 50, 1, 1, 100);
	currentBar.setStylePrimaryName("scrollbars");
...
//	verticalPanel.add(titleLabel);
	...
	verticalPanel.setStylePrimaryName("scrollbars");
	setGrid();
...
//    Изменены настройки цвета
	...
	if (positiveColor != null)
	    CircuitElm.positiveColor= Color.red;
	else if (getOptionFromStorage("alternativeColor", false))
	    CircuitElm.positiveColor = Color.red;
	if (negativeColor != null)
	    CircuitElm.negativeColor = Color.cyan;
    ...
    boolean shown = false;
    //Добавление компонентов(изменена конфигурация меню)
...
//Убрана ненужная часть функционала меню
...
//	    mouseElm = (CircuitElm) (editDialog.elm);
	...
	CircuitElm.selectColor = Color.orange;
...
//			ce.drawPost(g, ce.x2, ce.y2);
	...
	if (tempMouseMode==MODE_SELECT && mouseElm!=null) {
		mouseElm.drawHandles(g, Color.orange);
	}
	...
	if (dragElm != null &&
		      (dragElm.x != dragElm.x2 || dragElm.y != dragElm.y2)) {
		    	dragElm.draw(g);
		    	dragElm.drawHandles(g, Color.orange);
		}
...
//	console("setvoltages " + rs);
	...
	if ((menu=="edit" || menu=="main" || menu=="scopes") && noEditCheckItem.getState()) {
    	...
    	if (item=="clearpage") {
    	    	dialogShowing = new ClearDialog(this);
    	}
